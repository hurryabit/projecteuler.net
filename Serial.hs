...
serial' dat = [instance_context ["Serial"] "Serial" dat [ValD (VarP (mkName
    ...
    mkName "coseries") [(Clause [(VarP (mkName "rs")),(VarP (mkName "d"))]
    (NormalB (CompE ((map (\(ctorInd,ctor) -> (BindS (VarP (mkName ("t" ++
    show ctorInd))) (AppE (AppE (VarE (mkName ("alts" ++ show (ctorArity
    ctor)))) (VarE (mkName "rs"))) (VarE (mkName "d"))))) (id (zip
